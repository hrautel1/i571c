...
comprDeptEmployees employees dept = [x | x <- employees, dept == emplDept x]
...
employeesSalarySum employees = foldr (+) 0.0 (map  emplSalary employees)
...
quadraticRoots r1 r2 r3 = if delta < 0 then error "0" else ((x), (y))
                        where
                          x = equation + sqrt delta / (2 * r1)
                          y = equation - sqrt delta / (2 * r1)
                          delta = r2 * r2 - 4 * r1 * r3
                          equation = - r2 / (2 * r1)
...
infinite_num = numsFrom 0
  where numsFrom n = n : numsFrom (n+1)

expn x = zipWith (/) (map (x**) infinite_num) (map fact infinite_num)
  where 
    fact 0 = 1
...
charIndexes s c = [ val2 | (val1, val2) <- zip s [0..], val1 == c ]
...
foldTree treeFn leafFn tree =
  case tree of
    (Leaf tree) -> leafFn tree
    (Tree tree1 tree tree2) -> 
      treeFn (foldTree treeFn leafFn tree1) tree (foldTree treeFn leafFn tree2)
...
flattenTree tree_elements = foldTree (\leaf1 leaf leaf2 -> leaf1 ++ [ leaf] ++ leaf2) (\leaf -> [ leaf]) tree_elements
...
catenateTreeLists tree_elements =
  foldr (++) [] (flattenTree tree_elements)
